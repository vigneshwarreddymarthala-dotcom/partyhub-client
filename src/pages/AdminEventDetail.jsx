...
export default function AdminEventDetail() {
  ...
  // Mobile panel toggle: 'edit' | 'guests'
  ...
  async function fetchGuests() {
    ...
    const { data } = await supabase.from('rsvps').select('id, checked_in, created_at, user_id, profiles(full_name, username)').eq('event_id', eventId).order('created_at', { ascending: true });
    setGuests(data ?? []);
  ...
  const filteredGuests = guests.filter(g =>
    !search || (g.profiles?.full_name ?? '').toLowerCase().includes(search.toLowerCase()) || (g.profiles?.username ?? '').toLowerCase().includes(search.toLowerCase())
  );
  ...
  const GuestRoster = () => (
    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 sm:p-5 flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto space-y-1.5 scrollbar-hide min-h-0">
        {guestsLoading ? [...Array(4)].map((_, i) => <div key={i} className="h-10 rounded-lg bg-gray-800 animate-pulse" />)
          : filteredGuests.length === 0 ? <p className="text-gray-600 text-sm text-center py-6">No guests found</p>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 sm:py-8">
      ...
      <div className="lg:hidden">
        {mobilePanel === 'guests'
          ? <div className="h-[60vh]"><GuestRoster /></div>
          : <EditForm />}
      </div>
      ...
      <div className="hidden lg:grid lg:grid-cols-2 gap-6">
        <EditForm />
        <div className="h-[600px] flex flex-col"><GuestRoster /></div>
      </div>
    </div>
  );
}
