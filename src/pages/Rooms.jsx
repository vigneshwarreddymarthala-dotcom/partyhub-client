...
export default function Rooms() {
  ...
  const [kicked, setKicked] = useState(false); // user was removed from active room
  ...
  // Watch for RSVP deletions — if current user gets kicked, eject them
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes',
        ...
        (payload) => {
          // Remove the room whose event_id matches the deleted RSVP
          setRooms(prev => {
            const updated = prev.filter(r => r.event_id !== payload.old.event_id);
            return updated;
          });
          // If user is in that chat right now, eject them
          setActiveRoom(curr => {
            if (curr?.event_id === payload.old.event_id) {
              setKicked(true);
              return null;
            }
            return curr;
      ...
      <div className={`flex-1 flex flex-col min-w-0 ${screen === 'list' ? 'hidden sm:flex' : 'flex'}`}
        ...
        {kicked && (
          <div className="flex-1 flex flex-col items-center justify-center text-center px-8">
            <p className="text-5xl mb-4">🚫</p>
            <p className="text-white font-semibold text-lg">You've been removed</p>
            <p className="text-gray-500 text-sm mt-2">An admin removed you from this event's chat.</p>
            <button onClick={() => { setKicked(false); setScreen('list'); }}
              className="mt-5 px-4 py-2 rounded-xl bg-gray-800 hover:bg-gray-700 text-sm text-gray-300 transition-colors">
              Back to rooms
            </button>
          </div>
        )}
        ...
        {!kicked && activeRoom && (
          <>
            ...
            <div className="flex items-center gap-3 px-4 py-3 bg-gray-900/80 backdrop-blur border-b border-gray-800 shrink-0">
              <button onClick={() => setScreen('list')} className="sm:hidden p-1.5 -ml-1 text-gray-400 hover:text-white transition-colors">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500">
                  {isAdmin ? 'Admin view · all messages' : isEnded ? 'Event ended — read only' : 'Live chat'}
                </p>
              </div>
            </div>
                ...
                return (
                  <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'} ${sameUser ? 'mt-0.5' : 'mt-3'}`}>
                    <div className={`max-w-[78%] sm:max-w-[65%] flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                      ...
                      <div className={`relative px-3 py-2 rounded-2xl text-sm leading-relaxed break-words shadow-sm ${isMe ? 'bg-brand-600 text-white rounded-tr-sm' : 'bg-gray-800 text-gray-100 rounded-tl-sm'}`}>
                        {msg.content}
            ...
            {/* Input — disabled for ended events, always enabled for admin */}
            {isEnded && !isAdmin ? (
